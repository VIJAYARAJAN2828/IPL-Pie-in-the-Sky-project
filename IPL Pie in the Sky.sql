create database dbms2_finalproj;
use dbms2_finalproj;
...
with t1 as
(select bidder_id,count(bid_status='won') wins_of_each_bidder from ipl_bidding_details group by bidder_id), 
t2 as
(select distinct bidder_id,count(bid_status='won')over() tot_no_of_wins from ipl_bidding_details) 
select *,t1.wins_of_each_bidder*100/t2.tot_no_of_wins as percentage_of_wins 
from t1
join t2 using(bidder_id)
...
select stadium_id,stadium_name,city,count(*) no_of_matches_conducted from
(select * from ipl_match_schedule
join ipl_stadium
using(stadium_id)) t
...
with tot_matches as
(select stadium_id,stadium_name,count(*) no_of_matches_conducted from
(select * from ipl_match_schedule
join ipl_stadium
using(stadium_id)) t
group by stadium_id,stadium_name,city),                               #tot matches conducted in each stadium

tot_wins as
(select stadium_id,count(*) no_of_matchs_won_by_toss_winner  from
(select match_id,stadium_id,toss_winner,match_winner
from ipl_match_schedule
join ipl_match using(match_id)) t
where toss_winner=match_winner                                       #tot matches won by the toss winner
group by stadium_id)

select *, no_of_matchs_won_by_toss_winner*100/no_of_matches_conducted as percentage_wins_by_toss_winner
from tot_matches
join tot_wins using(stadium_id);


-- 4.	Show the total bids along with the bid team and team name.

select bid_team, team_name,count(bid_id) from
(select * 
from ipl_bidder_details i1
join ipl_bidding_details i2 using(bidder_id)
join ipl_team i3 on i2.bid_team=i3.team_id) t
group by bid_team, team_name;


-- 5.	Show the team ID who won the match as per the win details.
select match_id,team_id as team_won,win_details 
from ipl_match im
join ipl_team it on im.match_winner=it.team_id;


-- 6.	Display the total matches played, total matches won and total matches lost by the team along with its team name.
select team_id,team_name,sum(matches_won),sum(matches_lost)
from ipl_team it
...
select player_id,player_name,player_role,itp.remarks 
...
with t1 as
(select * from
(select team_id,count(*) no_of_allrounders
from ipl_team_players 
where player_role='all-rounder' 
group by team_id) t
where no_of_allrounders>4) 
select team_id,team_name,no_of_allrounders 
from t1
join ipl_team 
using(team_id)
...
select bidder_id, total_points, bid_status, stadium_name, win_details,year(match_date)
...
where win_details like '%CSK won%' and stadium_name='M. Chinnaswamy Stadium';
...
with temp1 as
(select *, dense_rank() over(order by wickets desc) as rnk from 
(select player_id,player_name,cast(substr(substr(performance_dtls,instr(performance_dtls,'wkt-'),6),5,2) as unsigned) as Wickets
from ipl_player where player_id in
(select player_id from ipl_team_players where player_role in ('bowler','all-rounder'))) t),
temp2 as 
(select *, (select player_role from ipl_team_players where player_id=temp1.player_id) as player_role from temp1
where rnk<=5),
temp3 as
(select *,(select team_id from ipl_team_players where player_id=temp2.player_id) as team_id from temp2)
select (select team_name from ipl_team where team_id=temp3.team_id) as team_name, player_name, player_role, Wickets  from temp3;
...
with temp1 as
(select bidder_name, count(*) as tot_bid_count from
(select bidder_name,bid_team, case when toss_winner=1 then team_id1 else team_id2 end as toss_winner_team
from ipl_match
join ipl_match_schedule using(match_id)
join ipl_bidding_details using(schedule_id)
join ipl_bidder_details using(bidder_id)) t
group by bidder_name),
temp2 as
(select bidder_name, count(*) as tot_bid_toss_win_count from
(select bidder_name,bid_team, case when toss_winner=1 then team_id1 else team_id2 end as toss_winner_team
from ipl_match
join ipl_match_schedule using(match_id)
join ipl_bidding_details using(schedule_id)
join ipl_bidder_details using(bidder_id)) t
where bid_team=toss_winner_team
group by bidder_name),
temp3 as
(select bidder_name,tot_bid_count, (select tot_bid_toss_win_count from temp2 where bidder_name=temp1.bidder_name) as tot_bid_toss_win_count
from temp1)
select *,(tot_bid_toss_win_count*100/tot_bid_count) as perc_toss_wins_of_bidder from temp3
order by (tot_bid_toss_win_count*100/tot_bid_count) desc;
...
select * from
(select *, max(duration) over() as max_duration from
(select tournmt_id, tournmt_name, datediff(to_date,from_date) as duration
from ipl_tournament) t) t1
...
select distinct bidder_id, bidder_name, year(bid_date) as yearr, month(bid_date) as monthh, total_points
...
order by monthh asc, total_points desc;



-- 14.	Write a query for the above question using sub-queries by having the same constraints as the above question.'
select * from
(select distinct bd.bidder_id,
(select bidder_name from ipl_bidder_details where bidder_id=bd.bidder_id) as bidder_name,
year(bid_date) as yearr,month(bid_date) as monthh,
(select total_points from ipl_bidder_points where bidder_id=bd.bidder_id) as total_points
...
select bidder_id, rnk_desc as ranks ,total_points, if(rnk_desc<=3,bidder_name,null) as Highest_3_bidders,
if(rnk_asc=1,bidder_name,null) as Lowest_3_Bidders from 
(select * from
(select *,dense_rank() over(order by total_points desc) as rnk_desc, dense_rank() over(order by total_points ) as rnk_asc from
(select distinct bidder_id,bidder_name,year(bid_date) as yearr,total_points from ipl_bidder_details 
join ipl_bidder_points using(bidder_id)
join ipl_bidding_details using(bidder_id)
where year(bid_date)=2018)t) t1
where rnk_desc<=3 or rnk_asc=1) t3
...
-- Here is one thing you should ensure whenever the new students' details come, you should also store them in the Student backup table 
-- so that if you modify the details in the student details table, you will be having the old details safely.
...
insert into table1 values(1,'A','a@gmail.com','0123456789'),
                         (2,'B','b@gmail.com','9876543210'),
                         (3,'C','c@gmail.com','4567891230'),
                         (4,'D','d@gmail.com','7894561230'),
                         (5,'E','e@gmail.com','3216549870');
...
for each row insert into table2 values(old.Student_id, old.Student_name,old.mail_id, old.mobile_no);

update table1 set mobile_no='6140358428' where student_id=3;
...
for each row insert into tab2 values(new.Student_id, new.Student_name,new.mail_id, new.mobile_no);

insert into tab1 values(1,'A','a@gmail.com','0123456789'),
                         (2,'B','b@gmail.com','9876543210'),
                         (3,'C','c@gmail.com','4567891230'),
                         (4,'D','d@gmail.com','7894561230'),
                         (5,'E','e@gmail.com','3216549870');
